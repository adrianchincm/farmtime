...
class MissingDailyStatUpdater < ApplicationService  
    ...
    def call
        ...
        seed_dates.each do |seed_date|
            PoolStatDaily.create(name: coindix_response["name"], tvl: seed_date["tvl"], apy: seed_date["apy"].to_f * 100, coindix_id: @coindix_id[:coindix_id], created_at: seed_date["date"])
        end        
    end
  end
